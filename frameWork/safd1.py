import tensorflow as tf
from tensorflow.contrib import rnn
import numpy as np
import json

def shuffle(X, T, C, pair_list):
    t = np.arange(len(X))
    s = np.arange(len(X))
    np.random.shuffle(s)
    t2s = dict(zip(s,t))
    new_pair = list()
    for i, pr in enumerate(pair_list):
        new_pair.append([t2s[pr[0]],t2s[pr[1]]])
    return np.array(X[s]), np.array(T[s]), np.array(C[s]), new_pair

def shuffle_pair(X):
    s = np.arange(len(X))
    np.random.shuffle(s)
    return np.array(X[s])


# parameters setting
n_input = 5
time_steps = 22
n_classes = 1

num_units = 128
learning_rate = .0003

batch_size = 128
# dataset_size = 5000
sigma = 30
theta = 0.3

# convert lstm units to class probability.
out_weights = tf.Variable(tf.random_normal([num_units, n_classes]))
out_bias = tf.Variable(tf.random_normal([n_classes]))
para_list = [out_weights, out_bias]

# input placeholder
X = tf.placeholder(tf.float32, [None, time_steps, n_input])
# T = tf.placeholder(tf.int32, shape=(None,))
C = tf.placeholder(tf.float32, shape=(None,))

# X_r = tf.placeholder(tf.float32, [None, time_steps, n_input])
# T_r = tf.placeholder(tf.int32, shape=(None,))

# [batch_size, n_steps, n_input] to "time_steps" number of [batch_size, n_input] tensors
input = tf.unstack(X,time_steps,axis=1)
# input_r = tf.unstack(X_r,time_steps,axis=1)

# define the network
lstm_layer = rnn.BasicLSTMCell(num_units, forget_bias=1)
outputs, _ = rnn.static_rnn(lstm_layer, input, dtype="float32")
# outputs_r, _ = rnn.static_rnn(lstm_layer, input_r, dtype="float32")

logits_series = tf.nn.sigmoid(tf.matmul(tf.reshape(outputs,[-1,num_units]),out_weights) + out_bias)
hazard_series = tf.reshape(logits_series, [-1,time_steps])

# hazard_series = tf.nn.l2_normalize(
#                     tf.reshape(logits_series, [-1,time_steps]),
#                     dim=1
#                 )




# logits_series_r = tf.nn.sigmoid(tf.matmul(tf.reshape(outputs_r,[-1,num_units]),out_weights) + out_bias)
# hazard_series_r = tf.reshape(logits_series_r, [-1,time_steps])


#loss
mle_mask = tf.placeholder(tf.float32, [None, time_steps])
mle_index = tf.placeholder(tf.int32, shape=(None,))
loss_mle = tf.subtract(
                tf.reduce_sum(
                    tf.multiply(hazard_series, mle_mask)),   # hazard_series: partial,  mle_mask: partial.
                                    tf.reduce_sum(tf.multiply(
                                                tf.log(
                                                    tf.gather(tf.reshape(hazard_series, [-1]), mle_index)   #  hazard_series: whole,  mle_index partial
                                                ),C  # C partial.
                                            )))

# loss_rank
i_ind = tf.placeholder(tf.int32, shape=(None,))
j_ind = tf.placeholder(tf.int32, shape=(None,))
loss_rank = tf.reduce_sum(
                tf.exp(
                    tf.divide(
                        tf.subtract(
                            tf.gather(tf.reshape(hazard_series, [-1]), j_ind),  # hazard_series, whole, j_ind partial.
                            tf.gather(tf.reshape(hazard_series, [-1]), i_ind)   # hazard_series, whole, i_ind partial.
                        ), sigma)
                )
            )

#train step
train_mle = tf.train.AdamOptimizer(learning_rate).minimize(loss_mle, var_list=para_list)
train_rank = tf.train.AdamOptimizer(learning_rate).minimize(loss_rank, var_list=para_list)
loss = theta*loss_mle + (1-theta)*loss_rank
train_mle_rank = tf.train.AdamOptimizer(learning_rate).minimize(loss, var_list=para_list)


# Input
dest_path = "../Data/"

X_train = np.load(dest_path + "X_train.npy")
T_train = np.load(dest_path + "T_train.npy")
C_train = np.load(dest_path + "C_train.npy")

X_mask = np.zeros([X_train.shape[0], X_train.shape[1]])
for v, t in zip(X_mask, T_train):
    for i in range(t):
        v[i] = 1
X_bases = np.multiply(np.arange(X_train.shape[0]), X_train.shape[1])
X_index = np.add(X_bases, np.subtract(T_train,1))



acc_pair = json.load(open("../Data/acc_pair.json", "r"))
pair_list = np.array([[int(i), j] for i, v in acc_pair.items() for j in v])[0:60000]

def usr2obsT(T):
    u2T = dict()
    for i, t in enumerate(T):
        u2T[i] = t
    return u2T

u2T = usr2obsT(T_train)

i_bases = pair_list[:,0]
j_bases = pair_list[:,1]

i_off = [u2T[i]-1 for i in i_bases]
# j_off = [u2T[j]-1 for j in j_bases]

i_index = i_bases*X_train.shape[1] + i_off
j_index = j_bases*X_train.shape[1] + i_off


X_test = np.load(dest_path + "X_test.npy")
T_test = np.load(dest_path + "T_test.npy")
C_test = np.load(dest_path + "C_test.npy")

X_event, T_event, X_censor, T_censor = list(), list(), list(), list()
for i, c in enumerate(C_test):
    if c == 1:
        X_event.append(X_test[i])
        T_event.append(T_test[i])
    else:
        X_censor.append(X_test[i])
        T_censor.append(T_test[i])
X_event, T_event, X_censor, T_censor = np.array(X_event), np.array(T_event), np.array(X_censor), np.array(T_censor)



sess = tf.Session()
sess.run(tf.global_variables_initializer())
q = np.divide(len(X_train), batch_size)
for n_epoch in range(10000):

    
    for n_batch in range(q):

        # _, _loss_mle = sess.run([train_mle, loss_mle],feed_dict={
        #                         X:X_train,
        #                         C:C_train,
        #                         mle_mask:X_mask,
        #                         mle_index:X_index})
        # #
        # _, _loss_rank = sess.run([train_rank, loss_rank],feed_dict={
        #                         X:X_train,
        #                         i_ind:i_index,
        #                         j_ind:j_index
        #                         })

        _, _loss, _loss_mle, _loss_rank = sess.run([train_mle_rank, loss, loss_mle, loss_rank],feed_dict={
                                            X: X_train,
                                            C: C_train,
                                            mle_mask: X_mask,
                                            mle_index: X_index,
                                            i_ind: i_index,
                                            j_ind: j_index})

    # print "epoch %s: %s" % (n_epoch, _loss_mle)

    # print "epoch %s: %s" % (n_epoch, _loss_rank)

    # print "epoch %s: %s %s %s" % (n_epoch, _loss, _loss_mle, _loss_rank)


    _hazard_series = np.array(
        sess.run([hazard_series], feed_dict={X:X_event})
    )
    _hazard_series = _hazard_series.reshape(_hazard_series.shape[1],_hazard_series.shape[2])

    T_pred = list()
    for hs in _hazard_series:
        # flag = True
        # for i, h in enumerate(hs):
        T_pred.append(np.argmax(hs)+1)
        #     if h > .25:
        #         T_pred.append(i+1)
        #         flag = False
        #         break
        # if flag:
        #     T_pred.append(22)

    print
    print T_event[0:100]
    print "-------------------"
    print T_pred[0:100]
    print
    mse = np.mean(np.abs(T_event-T_pred))
    print "epoch: %s"%n_epoch, mse, np.mean(T_pred), np.mean(T_event)

    # exit(0)


exit(0)















# predictions = _hazard_series.reshape((_hazard_series.shape[1], _hazard_series.shape[2]))
# T_pred = np.argmax(predictions, axis=1) + 1



















# Y = tf.nn.l2_normalize(logits_series,dim=1)
# logits_series = tf.nn.relu(tf.matmul(tf.reshape(outputs,[-1,num_units]),out_weights) + out_bias)
# hazard_series = tf.reshape(tf.nn.l2_normalize(logits_series,dim=1),[-1,time_steps])


# convert tensor to iterator by 'unstack'.
# H_iter = tf.unstack(hazard_series, batch_size, axis=0)
# T_iter = tf.unstack(T, batch_size, axis=0) # index starts from '1', so H_i index should subtract 1 by T_i.
# C_iter = tf.unstack(C, batch_size, axis=0)



# loss_mle = tf.reduce_mean(
#                 [tf.reduce_sum(tf.cast(H_i[0:T_i-1],tf.float32))
#                           -tf.multiply(tf.cast(C_i,tf.float32), tf.log(H_i[T_i-1]))
#                                                 for H_i, T_i, C_i in zip(H_iter, T_iter, C_iter)]
#                 )


# H_iter_r = tf.unstack(hazard_series_r, dataset_size, axis=0)
# T_iter_r = tf.unstack(T_r, dataset_size, axis=0)
#
# acc_len = 10
# Acc = tf.placeholder(tf.int32, shape=(acc_len,2)) # index starts from '0'
# acc_pair_iter = tf.unstack(Acc, acc_len, axis=0)

# print("here")
# loss_rank = tf.reduce_mean(
#                 [tf.exp(tf.divide(
#                     tf.gather(tf.gather(H_iter_r,pr[1]), tf.gather(T_iter_r,pr[0])-1) # H_iter_r is different,
#                                                                                       # and tf.gather(H_iter_r,pr[1]) is used to
#                                                                                       # pick up the corresponding user.
#                     - tf.gather(tf.gather(H_iter_r,pr[0]), tf.gather(T_iter_r,pr[0])-1), sigma))
#                                            for pr in acc_pair_iter])
# print("here over")
